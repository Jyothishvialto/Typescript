class IPAddressValidator {
  queryIP: string; // Declare type of queryIP as string

  constructor(queryIP: string) {
    this.queryIP = queryIP; // Assign input IP to the class property
  }

  validateIPAddress(): string { // Return type is string
    if (this.isValidIPv4()) {
  ...
  private isValidIPv4(): boolean { // Return type is boolean
    const parts: string[] = this.queryIP.split('.'); // Split IP by dot
    ...
    for (let part of parts) {
      ...
      if (part.length > 1 && part[0] === '0') // No leading zeros
        return false;
      const num: number = Number(part); // Convert part to number
  ...
  private isValidIPv6(): boolean { // Return type is boolean
    const parts: string[] = this.queryIP.split(':'); // Split IP by colon
    if (parts.length !== 8) // Must be exactly 8 parts
      return false;
    const hexRegex: RegExp = /^[0-9a-fA-F]{1,4}$/; // hexRegex: each part must be 1 to 4 hex digits
    for (let part of parts) {
