...
class Frequencycharacter {
    ...
    sortCharactersByFrequency() {
        let freq = {}; // Object to store character frequency counts
        for (let i = 0; i < this.s.length; i++) { // Loop through each character in the input string
            let ch = this.s[i]; // Get the current character
        ...
        let arr = []; // Array to store characters repeated by their frequency
        ...
        let result = arr.join(''); // Join all elements of array into a single string
        console.log("Sorted string by frequency:", result); // Output the final sorted string
    }
}
// Example usage:
const input = "tree"; // Sample input string
