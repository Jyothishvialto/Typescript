...
class PalindromeHelper {
    ...
    isPalindrome(str) {
        let left = 0; // Pointer from the start of the string
        let right = str.length - 1; // Pointer from the end of the string
    ...
    findLongestPalindrome(s) {
        let longest = ''; // Store the longest palindrome found
        for (let i = 0; i < s.length; i++) { // Outer loop - start index
            for (let j = i + 1; j <= s.length; j++) { // Inner loop - end index
                const substring = s.slice(i, j); // Get substring from i to j
...
const inputString = "babad"; // Sample input
const result = helper.findLongestPalindrome(inputString); // Call method
